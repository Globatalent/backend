...
function putFounds(username, amount) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      completedPayment.DebitedFunds.Amount = (parseFloat(amount) * 100).toString()
    ...
    try {
      ...
      userRepository.getUserByCriteria({username : username}, "investor")
        .then(userResult => {
          ...
          userResult.overview.totalFounds += (amount/100)
          userResult.overview.cashFounds += (amount/100)
          return userRepository.updateUser(userResult, "investor");
        })
        .then(result => {
